...
# ------------------------------------------------------------
# The permutation stage itself uses joblib/loky for true multi-process CPU work.
_pipeline_executor = concurrent.futures.ThreadPoolExecutor(max_workers=1)


def _permutation_worker(seed, coords, fil, n1, res, gain, n_clusters):
    rng = np.random.default_rng(seed)
    perm = rng.permutation(np.arange(len(coords)))
    A_idx = perm[:n1]
    B_idx = perm[n1:]

    MA, FA = core.build_mapper_from_indices(
        coords, fil, A_idx, res, gain, n_clusters, 0
    )
    MB, FB = core.build_mapper_from_indices(
        coords, fil, B_idx, res, gain, n_clusters, 0
    )
...
def _run_combo_sync(cc_df, imp_df, res, gain, knn_k, n_clusters, num_perm, n_jobs):
    """Run one complete (resolution, gain) combination outside Shiny's reactive graph."""
    start_time = time.perf_counter()

    coords, fil, idx_A, idx_B = core.preprocess_pool(
        cc_df, imp_df, knn_k=knn_k, use_2d_filter=True
    )
    if coords.size == 0:
        raise RuntimeError("No numeric columns found in uploaded datasets.")

    M_A, F_A = core.build_mapper_from_indices(
        coords, fil, idx_A, res, gain, n_clusters, 0
    )
    M_B, F_B = core.build_mapper_from_indices(
        coords, fil, idx_B, res, gain, n_clusters, 0
    )

    parts_obs = [
        core.bottleneck_statmapper((M_A, F_A), (M_B, F_B), topo)
        for topo in core.TOPO_TYPES
    ]
    ...
    perm_T = Parallel(
        ...
        delayed(_permutation_worker)(
            seed, coords, fil, n1, res, gain, n_clusters
        )
        for seed in range(num_perm)
    )

    perm_T = np.asarray(perm_T, dtype=float)
...
def server(input, output, session):
    ...
    current_combo_idx = reactive.Value(None)
    stop_requested = reactive.Value(False)
    ...
    async def _run_combo_task(cc_df, imp_df, res, gain, knn_k, n_clusters, num_perm, n_jobs):
    ...
    def _launch_next_combo():
        """Start the next pending combo without blocking the Shiny session."""
        if stop_requested():
            is_running.set(False)
            started_pipe.set(False)
            _append_log("Pipeline stopped after the current combo.")
            return
        ...
        if len(pending_idx) == 0:
            is_running.set(False)
            progress.set(1.0)
            _append_log("All parameter combos completed.")
            return

        i = int(pending_idx[0])
        res = int(grid.at[i, "res"])
        gain = float(grid.at[i, "gain"])

        grid.at[i, "status"] = "Running"
        param_grid.set(grid)
        current_combo_idx.set(i)

        _append_log(f"Running res={res}, gain={gain} ...")

        # Snapshot all reactive values BEFORE entering the background task.
        _run_combo_task(
            df_cc().copy(),
            df_imp().copy(),
            res,
    ...
    def _on_start_pipeline():
        ...
        if missing:
            _append_log(
                f"[PIPELINE][ERROR] tda_core missing: {', '.join(missing)}."
            )
            return
        ...
        # Validate the two analytical inputs before any heavy work starts.
        if df_cc().isna().any().any():
            _append_log("[PIPELINE][ERROR] Complete-case data contains missing values.")
            return
        if df_imp().isna().any().any():
            _append_log("[PIPELINE][ERROR] Imputed data contains missing values.")
            return
        ...
        # Reset state first, then write new log messages (so they are not erased).
        started_mapper.set(True)
        started_pipe.set(True)
        is_running.set(True)
        stop_requested.set(False)
        current_combo_idx.set(None)
        progress.set(0.0)
        live_log_lines.set([])
        ...
        _append_log(f"Pipeline started: {len(grid)} combo(s).")
        _append_log(
            f"Permutations={int(input.num_perm())}, "
            f"clusters={int(input.n_clusters())}, "
            f"parallel jobs={int(input.n_jobs())}."
        )

        _launch_next_combo()

    @reactive.effect
    def _handle_combo_task_state():
        """Move completed background work back into the reactive UI."""
        status = _run_combo_task.status()
        idx = current_combo_idx()

        if idx is None:
            return

        if status == "success":
            result = _run_combo_task.result()

            grid = param_grid().copy()
            if idx not in grid.index:
                return
            ...
            row = {
                ...
                "res": result["res"],
                "gain": result["gain"],
                "dataset_pair": "Complete vs Imputed",
            ...
            results_df.set(
                pd.concat([results_df(), pd.DataFrame([row])], ignore_index=True)
            )

            grid.at[idx, "status"] = "Done"
            param_grid.set(grid)

            done = int((grid["status"] == "Done").sum())
            failed = int((grid["status"] == "Failed").sum())
            total = len(grid)
            progress.set((done + failed) / total if total else 1.0)

            _append_log(
                f"res={result['res']}, gain={result['gain']} -> "
                f"Tobs={result['metric']:.4f}, "
            ...
            if idx in grid.index:
                ...
                _append_log(f"Error at res={res}, gain={gain}: {err}")
            ...
            is_running.set(False)
            started_pipe.set(False)
            _append_log("Pipeline task cancelled.")

    @reactive.effect
    @reactive.event(input.btn_stop)
    def _on_stop():
        # The currently running joblib batch is allowed to finish cleanly.
        # No new combo will be started afterwards.
        if not is_running():
            _append_log("Pipeline is not currently running.")
            return

        stop_requested.set(True)
        _append_log("Stop requested. Current combo will finish; no new combo will start.")


app = App(app_ui, server)
app.on_shutdown(_pipeline_executor.shutdown)
